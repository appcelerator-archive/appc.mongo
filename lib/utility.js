...
module.exports = function (Arrow, server) {
	...
	return {
		...
		/**
		 * Looks through the model instance for native Mongo types that we can parse.
		 * @param instance
		 */
		parseNativeTypes: function parseNativeTypes(instance) {
			...
			for (var key in fields) {
				if (fields.hasOwnProperty(key)) {
					...
					if (field.type === 'objectid') {
						...
						if (val && !(val instanceof ObjectID)) {
							instance[key] = new ObjectID(val.id || val); 
						}
					}
				}
			}
